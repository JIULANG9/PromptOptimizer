...
6. **状态管理**（Riverpod 3.x）：
   - 所有 Notifier 均继承 `Notifier<State>` 或 `AsyncNotifier<State>`，禁止使用 `StateNotifier`
   - 依赖项在 `build()` 方法中通过 `ref.watch()` 获取
   - 资源清理通过 `ref.onDispose()` 注册回调
   - 全局持久 Provider（settingsProvider/optimizationProvider/toastProvider）使用 `ref.keepAlive()`
   - 高频更新场景使用 `select()` 精细化监听，减少不必要的 rebuild
   - Provider 遵循就近原则
